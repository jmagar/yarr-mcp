...
- Extended the curated ArrManager command surface to the v1 (music/book) kinds Lidarr and Readarr (C3). The full curated arr command set — reads (`quality_profiles`, `list`, `wanted`, `queue`, `history`, `rootfolders`, `health`) and writes (`set_quality`, `search`, `refresh`, `monitor`, `unmonitor`, `add`, `delete`) — now works for Lidarr (`/api/v1`, `artist`) and Readarr (`/api/v1`, `author`) on both surfaces, with no new per-kind wiring. Everything is driven by `ServiceKind::descriptor()`: the api prefix (`/api/v1` vs `/api/v3`), the resource noun (`artist`/`author` vs `series`/`movie`), the editor id key (`{noun}Ids` => `artistIds`/`authorIds`), and now the `/command` names — `(search, refresh)` are looked up from a `COMMAND_NAMES` table keyed by resource noun (`ArtistSearch`/`RefreshArtist`, `AuthorSearch`/`RefreshAuthor`) rather than the previous v3-only `series`/`movie` branch (the names are not uniform across the family — Radarr pluralises to `MoviesSearch`). Command applicability is expressed through `Capability::ArrManager` (shared by all four arr kinds) plus a NEW typed `KindDescriptor.has_metadata_profiles` flag (true for Lidarr/Readarr only) — NOT a per-(action, kind) deny list. `set_quality` targets the quality-profile axis that all four kinds share, so it applies cleanly; the typed flag documents the additional metadata-profile axis the v1 kinds carry and is the seam a future metadata command would gate on. Strict v1/v3 separation is preserved (security S7): the per-kind `path_allowlist` keeps Lidarr/Readarr on `/api/v1` only, so `/api/v3/artist` on Lidarr is rejected at `validate_service_path`. Touches `src/capability.rs` (typed flag), `src/app/arr/editor.rs` (table-driven command names); v3 sonarr/radarr behaviour is unchanged.
