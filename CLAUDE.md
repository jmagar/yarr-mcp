...
| `src/codemode/dts.rs` | **Agent-facing** TypeScript surface (`codemode_dts()`): hand-written API declarations + per-service `declare namespace` response interfaces generated from the models' `JsonSchema` (the preamble is sandbox-only and never reaches the agent — this does, via the `rustarr://schema/codemode` MCP resource + the tool description) |
