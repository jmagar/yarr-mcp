...
2. **`src/actions/commands/<cap>.rs`** — append a `CommandDescriptor` to the capability's const slice: `name` (globally-unique snake_case action), `capability`, `description`, `required_scope`, `required_params`/`optional_params`, `confirm_required`, `mutates`, and the `handler`. **`confirm_required` now marks a *destructive delete*** — it is the SSOT for `action_is_destructive` and the ONLY thing still gated (MCP elicitation / CLI `--confirm`). Set it `true` only for destructive deletes; every other write keeps `mutates=true, confirm_required=false` and runs immediately. The invariant is **`confirm_required ⇔ destructive`** (and a gated command must `mutate`) — enforced by `tests/parity.rs`. The slice is concatenated at the single extension point in `src/actions/registry.rs::build_curated_commands` — no enum/match edits.
...
descriptor, and (e) `confirm_required ⇔ destructive` (only destructive deletes are
gated; a gated command must also `mutate`). MCP resources and prompts are
