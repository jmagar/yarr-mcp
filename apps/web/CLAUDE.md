...
A Next.js 16 frontend (static export) that serves the rustarr demo UI. It connects to the `rustarr` MCP server's REST API and provides interactive tooling, an API explorer, and a dashboard.

Use `apps/web` only for application/platform servers that intentionally expose API + CLI + MCP + Web (for rustarr `axon`, `lab`, and `syslog`). Upstream-client MCP servers such as `unrust`, `rustifi`, `rustify`, `rustscale`, and `apprise` should keep MCP + CLI parity and omit REST/Web unless they own additional workflows, state, dashboards, or non-MCP consumers.
...
│   └── api.ts              # Typed REST client for rustarr server
...
- **API calls go through `lib/api.ts`** — do not fetch the rustarr server directly from page components; add methods to `api.ts` and call those.
