...
The xtask starts a local Rustarr MCP server against `/home/jmagar/.rustarr-shart/.env`, discovers the advertised service tool schemas through `mcporter list --schema`, and calls each advertised action through `mcporter call`. It must not target production service credentials. Shart is a disposable fake stack, so confirmed media-stack writes/removes/deletes are expected live coverage.
