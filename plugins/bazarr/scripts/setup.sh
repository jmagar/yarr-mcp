#!/usr/bin/env bash
# SessionStart / ConfigChange hook for the bazarr plugin.
# Persists only manifest-declared options as non-executable JSON.
set -euo pipefail

CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/lab-bazarr"
CONFIG_FILE="${CONFIG_DIR}/config.json"
ALLOWED_KEYS=("BAZARR_URL" "BAZARR_API_KEY")

command -v node >/dev/null 2>&1 || {
  printf 'bazarr: node is required to persist plugin settings safely\n' >&2
  exit 1
}
mkdir -p "${CONFIG_DIR}"
...
if [[ "${count}" -eq 0 ]]; then
  printf 'bazarr: not configured yet - set the service URL/key in plugin settings.\n' >&2
else
  printf 'bazarr: wrote %s settings to %s\n' "${count}" "${CONFIG_FILE}"
fi
