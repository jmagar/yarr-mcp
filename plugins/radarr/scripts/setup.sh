...
ALLOWED_KEYS=("RADARR_URL" "RADARR_API_KEY" "RADARR_DEFAULT_QUALITY_PROFILE")

command -v node >/dev/null 2>&1 || {
  printf 'radarr: node is required to persist plugin settings safely\n' >&2
  exit 1
}
mkdir -p "${CONFIG_DIR}"
...
if [[ "${count}" -eq 0 ]]; then
  printf 'radarr: not configured yet - set the service URL/key in plugin settings.\n' >&2
else
  printf 'radarr: wrote %s settings to %s\n' "${count}" "${CONFIG_FILE}"
fi
