...
CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/lab-sabnzbd/config.env"
...
: "${SABNZBD_URL:?set it in the plugin settings}"
: "${SABNZBD_API_KEY:?set it in the plugin settings}"

# Use SABNZBD_URL for consistency with .env
SAB_URL="$SABNZBD_URL"
...
cmd_add() {
    ...
    # api_call percent-encodes each value via jq @uri, so pass the RAW url
    # below. Pre-encoding it would double-encode (%3A -> %253A) and corrupt addurl.
    
    local params=("name=$url")
