...
```bash
# Get first 10 items
curl -s "$SABNZBD_URL/api?mode=queue&start=0&limit=10&output=json&apikey=$SABNZBD_API_KEY" | jq
```
...
```bash
curl -s "$SABNZBD_URL/api?mode=queue&output=json&apikey=$SABNZBD_API_KEY" | jq '.queue | {speed, speedlimit, speedlimit_abs}'
```
...
```bash
# All history
curl -s "$SABNZBD_URL/api?mode=history&output=json&apikey=$SABNZBD_API_KEY" | jq

# Last 20 items
curl -s "$SABNZBD_URL/api?mode=history&limit=20&output=json&apikey=$SABNZBD_API_KEY" | jq
```

### Get Failed Downloads Only

```bash
curl -s "$SABNZBD_URL/api?mode=history&failed_only=1&output=json&apikey=$SABNZBD_API_KEY" | \
...
```bash
curl -s "$SABNZBD_URL/api?mode=history&category=movies&output=json&apikey=$SABNZBD_API_KEY" | jq
```
...
# Clear all history
curl -X POST "$SABNZBD_URL/api?mode=history&name=delete&value=all&apikey=$SABNZBD_API_KEY"
```
...
curl -X POST "$SABNZBD_URL/api?mode=queue&name=priority&value=SABnzbd_nzo_abc123&value2=1&apikey=$SABNZBD_API_KEY"
...
```bash
curl -s "$SABNZBD_URL/api?mode=fullstatus&output=json&apikey=$SABNZBD_API_KEY" | jq
```

### Get Server Statistics

```bash
curl -s "$SABNZBD_URL/api?mode=server_stats&output=json&apikey=$SABNZBD_API_KEY" | jq '{total, month, week, day}'
```

### Get Configuration

```bash
curl -s "$SABNZBD_URL/api?mode=get_config&output=json&apikey=$SABNZBD_API_KEY" | jq
```
...
3. **Check history when complete:**
   ```bash
   curl -s "$SABNZBD_URL/api?mode=history&limit=1&output=json&apikey=$SABNZBD_API_KEY" | \
...
watch -n 5 "curl -s '$SABNZBD_URL/api?mode=queue&output=json&apikey=$SABNZBD_API_KEY' | \
...
curl -X POST "$SABNZBD_URL/api?mode=speedlimit&value=2048&apikey=$SABNZBD_API_KEY"
...
```bash
curl -s "$SABNZBD_URL/api?mode=history&limit=5&output=json&apikey=$SABNZBD_API_KEY" | \
