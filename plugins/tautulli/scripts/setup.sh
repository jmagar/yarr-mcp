...
CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/lab-tautulli"
CONFIG_FILE="${CONFIG_DIR}/config.json"
ALLOWED_KEYS=("TAUTULLI_URL" "TAUTULLI_API_KEY")

command -v node >/dev/null 2>&1 || {
  printf 'tautulli: node is required to persist plugin settings safely\n' >&2
  exit 1
}
mkdir -p "${CONFIG_DIR}"
...
if [[ "${count}" -eq 0 ]]; then
  printf 'tautulli: not configured yet - set the service URL/key in plugin settings.\n' >&2
else
  printf 'tautulli: wrote %s settings to %s\n' "${count}" "${CONFIG_FILE}"
fi
