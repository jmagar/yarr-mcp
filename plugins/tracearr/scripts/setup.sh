...
CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/lab-tracearr"
CONFIG_FILE="${CONFIG_DIR}/config.json"
ALLOWED_KEYS=("TRACEARR_URL")

command -v node >/dev/null 2>&1 || {
  printf 'tracearr: node is required to persist plugin settings safely\n' >&2
  exit 1
}
mkdir -p "${CONFIG_DIR}"
...
if [[ "${count}" -eq 0 ]]; then
  printf 'tracearr: not configured yet - set the service URL/key in plugin settings.\n' >&2
else
  printf 'tracearr: wrote %s settings to %s\n' "${count}" "${CONFIG_FILE}"
fi
