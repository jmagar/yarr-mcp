...
CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/lab-prowlarr/config.json"
# shellcheck source=/dev/null
source "$(dirname "${BASH_SOURCE[0]}")/load-config.sh"
load_plugin_config "$CONFIG_FILE" PROWLARR_URL PROWLARR_API_KEY

: "${PROWLARR_URL:?set it in the plugin settings}"
: "${PROWLARR_API_KEY:?set it in the plugin settings}"

# Remove trailing slash
PROWLARR_URL="${PROWLARR_URL%/}"

require_num() { [[ "${1:-}" =~ ^[0-9]+$ ]] || { echo '{"error":"expected a numeric id"}' >&2; exit 2; }; }
require_imdb() { [[ "${1:-}" =~ ^tt[0-9]+$ ]] || { echo '{"error":"imdb id must look like tt1234567"}' >&2; exit 2; }; }
...
cmd_tv_search() {
    ...
    # Ids are embedded in the Prowlarr query operators — keep them numeric.
    [[ -n "$tvdb" ]] && require_num "$tvdb"
    [[ -n "$season" ]] && require_num "$season"
    [[ -n "$episode" ]] && require_num "$episode"
...
cmd_movie_search() {
    ...
    # Validate ids before embedding them in the Prowlarr query operators.
    [[ -n "$tmdb" ]] && require_num "$tmdb"
    [[ -n "$imdb" ]] && require_imdb "$imdb"
