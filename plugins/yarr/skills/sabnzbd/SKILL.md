...
Credentials are configured in the **plugin settings** (userConfig). A `SessionStart` hook writes them to `~/.config/lab-sabnzbd/config.json`, which the scripts parse automatically — no manual file editing. Variables used:
