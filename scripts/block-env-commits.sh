...
#   file (other than .env.example). This prevents accidentally committing API
...
# Find any .env* files in the staged list, excluding .env.example.
# Regex breakdown:
...
if [[ -n "$blocked" ]]; then
    ...
    echo "" >&2
    echo "Only .env.example is allowed to be committed." >&2
