...
#   file (other than an exact `.env.example`). This prevents accidentally committing API
#   keys, OAuth secrets, or bearer tokens.
#
# What it allows:
#   - .env.example  (the committed template — no real secrets)
#
# What it blocks:
...
# Find any .env* files in the staged list, excluding exact `.env.example` files.
# Regex breakdown:
...
if [[ -n "$blocked" ]]; then
    ...
    echo "" >&2
    echo "Only an exact .env.example template is allowed to be committed." >&2
