...
#[tokio::test]
async fn scan_requires_confirm() {
    // The write guard runs before the capability/transport, so an unreachable plex
    // still surfaces the confirm error first.
    let svc = service_with(&[("plex", ServiceKind::Plex)]);
    let err = svc
        .media_scan("plex", Some("3"), false)
        .await
        .expect_err("media_scan without confirm must be rejected");
    assert!(err.to_string().contains("confirm"), "got: {err}");
}

#[tokio::test]
async fn scan_rejects_non_media_kind_even_with_confirm() {
    // With confirm=true the next gate is the capability check, which must reject a
    // non-media kind before any request is built.
    let svc = service_with(&[("sonarr", ServiceKind::Sonarr)]);
    let err = svc
        .media_scan("sonarr", Some("3"), true)
        .await
...
async fn plex_scan_requires_library_id() {
    ...
    // missing-library error surfaces after confirm + capability pass.
    let svc = service_with(&[("plex", ServiceKind::Plex)]);
    let err = svc
        .media_scan("plex", None, true)
        .await
