...
async def get_sdwan_config_by_id(config_id: str) -> Dict[str, Any]:
    """Retrieves details for a specific SD-WAN configuration by its ID (EA)."""
    if not config_id:
        raise ValueError("config_id is required.")
    return await _make_api_request("GET", f"/ea/sdwan/configs/{config_id}")


@mcp.tool()
async def get_sdwan_config_status(config_id: str) -> Dict[str, Any]:
    """Retrieves the status for a specific SD-WAN configuration by its ID (EA)."""
    if not config_id:
        raise ValueError("config_id is required.")
...
async def mcp_server_health_check() -> Dict[str, Any]: # Removed ctx: Context as it wasn't used in original Unifi health check
    ...
    """
    logger.info("MCP server health check requested for Unifi.")
    service_name = "unifi" # Define service name

    mcp_configured = all([UNIFI_API_KEY, UNIFI_BASE_URL])
    if not mcp_configured:
        logger.error("Unifi API Key or Base URL not configured for the MCP server.")
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": False, "reason": "Unifi API Key or Base URL not configured for MCP server."}

    try:
        # Perform a lightweight API call - list_hosts with pageSize=1 and is_paginated=False
        # to confirm API key validity and connectivity.
        logger.debug("Attempting to call Unifi /v1/hosts endpoint (pageSize=1) for health check...")
        response = await _make_api_request(
            method="GET", 
            endpoint="/v1/hosts", 
            params={"pageSize": "1"}, # API expects string for pageSize
            is_paginated=False # Important: ensures only one call and direct data return
        )
        
        # _make_api_request is designed to return the 'data' part of the Unifi API response upon success,
        # or raise an exception if the API call itself fails (HTTP error, etc.) or if the API returns an error structure.
        # A successful call to /v1/hosts (even with pageSize=1) should return a list (possibly empty) within the 'data' field.
        if isinstance(response, list): # Successful if response is a list (of hosts, even if empty)
            logger.info(f"Unifi API accessible. Health check (list_hosts with pageSize=1) successful. Found {len(response)} host(s) in first check.")
            return {
                "status": "ok", 
                ...
                "details": {
                    "hosts_in_check": len(response),
                    "message": "Unifi API is responsive. Host list check successful."
                }
            }
        # The Unifi API for /v1/hosts should return a list. If it's a dict, it might be an unexpected success or a wrapped error.
        # However, _make_api_request is supposed to unwrap 'data' or raise an error.
        # This case handles if _make_api_request itself returned an unexpected dict structure (not an error it raised).
        elif isinstance(response, dict):
             logger.warning(f"Unifi API accessible but health check (list_hosts with pageSize=1) returned an unexpected dictionary: {response}. This might indicate an issue or an empty successful response not conforming to list.")
             # Treating as potentially okay but with a warning, assuming the API call itself didn't raise an exception via _make_api_request
             return {
                "status": "ok", # Or "warning" if we had such a status
                "service_name": service_name,
                "service_accessible": True, 
                "mcp_server_configured": True,
                "details": {
                    "message": "Unifi API is responsive but returned an unexpected dictionary structure for host list check.",
                    "raw_response": response
                }
            }
        else: # Should not be reached if _make_api_request behaves as expected (returns list/dict or raises error)
            logger.error(f"Unifi API health check returned an unhandled data structure: {type(response)} - {response}")
            return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": True, "reason": f"Unifi API returned unexpected data structure: {type(response)}"}
            
    except ValueError as ve: # From _make_api_request if API_KEY is missing, or from tool arg validation (not here)
        logger.error(f"Configuration or Value Error during Unifi health check: {ve}")
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": False, "reason": f"Configuration error: {str(ve)}"}
    except httpx.HTTPStatusError as hse:
        error_detail = hse.response.text
        try: 
            parsed_error = hse.response.json()
            # Try to get specific Unifi error details
            error_code = parsed_error.get('code', 'UNKNOWN_CODE')
            error_msg = parsed_error.get('message', hse.response.text)
            http_status = parsed_error.get('httpStatusCode', hse.response.status_code)
            error_detail = f"({http_status}) {error_code} - {error_msg}"
        except Exception: # Keep original text if parsing fails
            pass 
        logger.error(f"HTTPStatusError during Unifi health check: {hse.response.status_code} - {error_detail}", exc_info=True)
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": True, "reason": f"Unifi API HTTP Error: {error_detail}"}
    except httpx.RequestError as re: # Covers network issues, DNS failures, etc.
        logger.error(f"RequestError during Unifi health check: {re}", exc_info=True)
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": True, "reason": f"Unifi API Request Error: {str(re)}"}
    except Exception as e: # Catch-all for other unexpected errors from _make_api_request or this function
        logger.exception("Unexpected exception during Unifi health check")
