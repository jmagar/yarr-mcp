...
async def mcp_server_health_check() -> Dict[str, Any]:
    ...
    """
    logger.info("MCP server health check requested for Unraid.")
    service_name = "unraid" # Define service name

    if not UNRAID_API_URL or not UNRAID_API_KEY:
        logger.error("UNRAID_API_URL or UNRAID_API_KEY is not configured for the MCP server.")
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": False, "reason": "Unraid URL or API Key not configured for MCP server."}

    # Perform a lightweight GraphQL query, e.g., get basic OS info or Unraid version from system info
    health_query = """
    query GetUnraidHealth {
      info {
        os { release codename }
        versions { unraid }
      }
    ...
    """
    try:
        logger.debug("Attempting to call Unraid GraphQL API for health check...")
        response_data = await _make_graphql_request(health_query)
        # _make_graphql_request returns the "data" part of the GraphQL response or raises an exception.
        
        info = response_data.get("info")
        if info and info.get("versions") and info["versions"].get("unraid"):
            unraid_version = info["versions"]["unraid"]
            os_release = info.get("os", {}).get("release", "N/A")
            os_codename = info.get("os", {}).get("codename", "N/A")
            logger.info(f"Unraid instance accessible. Version: {unraid_version}, OS: {os_release} ({os_codename})")
            return {
                "status": "ok", 
                ...
                "details": {
                    ...
                    "message": "Unraid instance is responsive."
                }
            }
        elif info: # Got some info, but not the full expected structure
            logger.warning(f"Unraid instance accessible but health check query returned partial data: {info}")
            return {
                "status": "ok", 
                "service_name": service_name,
                "service_accessible": True, 
                "mcp_server_configured": True,
                "details": {
                    "message": "Unraid instance is responsive but returned partial data.",
                    "raw_response": info
                }
            }
        else: # No "info" field or unexpected structure from _make_graphql_request
            logger.warning(f"Unraid health check did not return expected 'info' structure: {response_data}")
            return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": True, "reason": "Unraid API did not return expected data for health check."}
    
    except (ConnectionError, ValueError, httpx.TimeoutException) as e: # Specific errors from _make_graphql_request or httpx
        logger.error(f"Connection or API error during Unraid health check: {e}", exc_info=True)
        return {"status": "error", "service_name": service_name, "service_accessible": False, "mcp_server_configured": True, "reason": f"Failed to connect to Unraid, API key invalid, or API error: {str(e)}"}
    except Exception as e: # Catch-all for other unexpected errors
        logger.exception("Unexpected exception during Unraid health check")
