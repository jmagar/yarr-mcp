...
describe("LogService", () => {
  it("reads only the fixed Yarr log with fixed byte and line bounds", async () => {
    ...
    lines[599] = "\u001b[31merror log-secret log-secret\u001b[0m\u0000\u0007";
    const reader: BoundedLogReader = {
      readTail: vi.fn(async () => ({ text: `${lines.join("\n")}\n`, bytesTruncated: true })),
    };
    const service = new LogService(
      reader,
      new StoredSecretRedactor({ currentSecrets: async () => ["", "log-secret", "log"] }),
    );
    ...
    expect(result.lines.at(-1)).toBe("error [REDACTED] [REDACTED]");
  ...
  it("does not expose a caller-controlled path", () => {
    const service = new LogService(
      ...
      { redactMany: async (values) => [...values] },
    );
