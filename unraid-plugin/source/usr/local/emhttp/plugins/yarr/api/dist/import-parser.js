...
function parseImportText(text) {
    if (text.includes("\u0000"))
        throw new Error("import text contains an invalid NUL byte");
...
function parseEnvironmentValue(raw, line) {
...
function parseToml(text) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (header) {
            ...
            service = undefined;
            if (tables.has(table))
                throw new Error(`duplicate TOML table on line ${index + 1}`);
...
function validateNonServiceTables(tables, warnings) {
    for (const [table, fields] of tables) {
        for (const [field, value] of fields) {
            ...
            if (!supported.has(field))
                throw new Error(`unsupported Yarr TOML field [${table}].${field}`);
            warnings.push(`Yarr TOML field [${table}].${field} is valid but is not imported as a service setting`);
        }
    }
}
function importTomlServices(services, warnings) {
    const values = Object.create(null);
    const seen = new Set();
    for (const [index, service] of services.entries()) {
        ...
        if (seen.has(entry.id))
            throw new Error(`duplicate Yarr TOML service ${entry.id}`);
        seen.add(entry.id);
        if (kind !== undefined && name !== undefined) {
            ...
            if (!named)
                warnings.push(`Yarr TOML display name for ${entry.id} was not imported`);
        }
        let imported = 0;
        ...
        imported += assignServiceValue(values, entry.apiKeyKeys, apiKey ?? token, apiKey === undefined ? "token" : "api_key", entry.id);
        if (imported === 0) {
...
function resolveService(value, position) {
    const entry = value === undefined ? undefined : resolveOptionalService(value);
    if (!entry)
        throw new Error(`unsupported or missing Yarr TOML service identity at entry ${position}`);
    return entry;
}
function resolveOptionalService(value) {
    const normalized = (0, service_catalog_1.normalizeCatalogKey)(value).replace(/^YARR_/, "").toLowerCase();
    return service_catalog_1.SERVICE_CATALOG_BY_ID.get(normalized);
}
function assignServiceValue(values, keys, value, field, service) {
    if (value === undefined)
        return 0;
    if (keys.length === 0)
        throw new Error(`Yarr TOML service ${service} does not support ${field}`);
    if (Object.hasOwn(values, keys[0]))
        throw new Error(`duplicate Yarr TOML service value for ${service}`);
    values[keys[0]] = value;
    return 1;
}
function optionalTomlString(fields, field) {
    const value = fields.get(field);
    if (value === undefined)
        return undefined;
    if (typeof value !== "string")
        throw new Error(`Yarr TOML service field ${field} must be a string`);
    return value;
}
    ...
    if (value.startsWith("[") && value.endsWith("]")) {
        ...
        return members.map((member) => {
            ...
            if (Array.isArray(parsed))
                throw new Error(`nested TOML arrays are unsupported on line ${line}`);
            return parsed;
        });
    }
    throw new Error(`unsupported TOML value on line ${line}`);
}
function splitTomlArray(raw, line) {
    if (raw.trim() === "")
        return [];
    const result = [];
    let start = 0;
    ...
    for (let index = 0; index < raw.length; index += 1) {
        ...
        if (quote === '"') {
            ...
            if (member === "")
                throw new Error(`invalid TOML array on line ${line}`);
            result.push(member);
            start = index + 1;
        }
    }
    if (quote !== "")
        throw new Error(`unterminated TOML array string on line ${line}`);
